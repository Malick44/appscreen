...
5. Set the required application settings and one complete Storage credential pair below through protected runtime settings. Disable **Build Variable** for credentials and **Inject Build Args to Dockerfile**; this image needs no application secrets during build. Do not paste resolved Compose output, environment dumps or credentials into tickets/logs. [Coolify environment-variable guidance](https://coolify.io/docs/knowledge-base/environment-variables)
...
| `SUPABASE_URL` | Intended Auth instance's HTTPS origin |
| `SUPABASE_PUBLISHABLE_KEY` | Matching public Auth key |
| Storage credential pair | For one installation, set `SUPABASE_SERVICE_ROLE_KEY` matching `SUPABASE_URL`. For split Auth/Storage, set both `SUPABASE_STORAGE_URL` and `SUPABASE_STORAGE_SERVICE_ROLE_KEY`. |

For split Storage, use a canonical HTTPS origin without a trailing slash or path. Both processes receive the same paired overrides; a partial pair is rejected at startup before Storage access. Auth continues using its own URL/public key. With service email disabled by this Compose file, the Auth admin key (`SUPABASE_SERVICE_ROLE_KEY`) may be empty when a complete Storage override is configured. Storage credentials remain installation-wide even when AppScreen uses only one private bucket. Review that scope before selecting shared Storage. Compose expansion checks the five mandatory application settings; the startup verifier additionally requires a complete, valid Storage pair.
